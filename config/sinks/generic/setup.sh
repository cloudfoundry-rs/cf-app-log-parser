...
does_sink_exist() {
    ...
    for sink in /entrypoints/generic/*; do
...
SINK_COUNT=$(
    set -f -- $INCLUDED_SINKS
    echo $#
)

echo "Fount $SINK_COUNT sinks to install by generic sink"

for sink_to_check in $INCLUDED_SINKS; do
    does_sink_exist $sink_to_check

    if [[ $sink_to_check == "generic" ]]; then
        echo "You are not allowed to install the generic sink using the generic sink"
        exit 1
    fi
done

for sink_to_install in $INCLUDED_SINKS; do
    install_sink $sink_to_install
done

PORT=8080 /usr/local/bin/vector validate $CONFIG_PATH --no-environment --deny-warnings
