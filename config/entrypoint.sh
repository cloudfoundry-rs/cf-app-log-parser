...
if [ $ENTRYPOINT_FILES_COUNT -gt 0 ]; then
    for entrypoint_file in $(ls /entrypoints/*.sh); do
...
if [ "$INGEST_AUTH_STRATEGY" == "basic" ]; then
    echo "Using basic authentication"
    if [ -z "$(env INGEST_AUTH_SERVICE_NAME)" ]; then
    ...
    if
        [ -z "$(env INGEST_AUTH_SERVICE_NAME)" ]
